...
module System.Taffybar.MyWorkspaceSwitcher (
  -- * Usage
  -- $usage
  wspaceSwitcherNew
) where
...
import Data.List ((\\), findIndices, sortBy, sort)
...
urgentCallback cfg deskRef event = Gtk.postGUIAsync $ do
  ...
  withDefaultCtx $ do
    ...
    when isUrgent $ do
      ...
      visibleWs <- getVisibleWorkspaces
      nonEmptyWs <- mapM getWorkspace =<< getWindows
      when (this /= that) $ liftIO $ do
        toggleUrgent deskRef that True
        mark desktop pad (urgentWorkspace cfg) (lastWorkspaceInSwitcher nonEmptyWs visibleWs) that
...
transition cfg desktop wss = do
  ...
  let urgentWs = map WSIdx $ findIndices urgent desktop
      ...
      pad = if workspacePad cfg then prefixSpace else id
      lastWs = lastWorkspaceInSwitcher nonEmpty wss
  mapM_ (mark desktop pad (hiddenWorkspace cfg) lastWs) nonEmptyWs
  mapM_ (setWidgetNames desktop "hidden") nonEmptyWs
  mapM_ (mark desktop pad (emptyWorkspace cfg) lastWs) (allWs \\ nonEmpty)
  mapM_ (setWidgetNames desktop "empty") (allWs \\ nonEmpty)
  case wss of
    active:rest -> do
      mark desktop pad (activeWorkspace cfg) lastWs active
      setWidgetNames desktop "active" active
      mapM_ (mark desktop pad (visibleWorkspace cfg) lastWs) rest
      mapM_ (setWidgetNames desktop "visible") rest
    _ -> return ()
  mapM_ (mark desktop pad (urgentWorkspace cfg) lastWs) urgentWs
...
mark :: Desktop            -- ^ List of all available labels.
     ...
     -> WorkspaceIdx       -- ^ Index of the last workspace displayed in the switcher.
     -> WorkspaceIdx       -- ^ Index of the Label to modify.
     -> IO ()
mark desktop pad decorate lastWsIdx wsIdx
  | Just ws <- getWS desktop wsIdx = do
    Gtk.postGUIAsync $ Gtk.labelSetMarkup (label ws) $ pad $ decorate $ name' lastWsIdx wsIdx (name ws)
  | otherwise = return ()

lastWorkspaceInSwitcher :: [WorkspaceIdx] -> [WorkspaceIdx] -> WorkspaceIdx
lastWorkspaceInSwitcher nonEmptyWs visibleWs =
  last . sort $ nonEmptyWs ++ visibleWs

name' :: WorkspaceIdx -> WorkspaceIdx -> String -> String
name' (WSIdx lastIdx) (WSIdx idx) name =
  (show $ idx + 1) ++ ": " ++ name ++ lastMarker
  where lastMarker = if idx == lastIdx then "\x00a0" else ""
